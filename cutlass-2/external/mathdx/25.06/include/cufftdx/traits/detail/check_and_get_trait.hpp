    ...
    namespace detail {
        ...
        namespace get_block_config_impl {
            ...
            class helper_block_ct
            {
                ...
                static constexpr bool         this_fft_is_folded = real_fft_mode_of<Description>::value == real_mode::folded;
                static constexpr unsigned int this_fft_size_v            = this_fft_is_folded ? size_of<Description>::value / 2 : size_of<Description>::value;
                ...
                static constexpr auto this_fft_type_v                    = this_fft_is_folded ? fft_type::c2c : type_of<Description>::value;
                ...
                // Get optimal (EPT, FPB)
                using optimal_ept = CUFFTDX_STD::conditional_t<this_fft_is_folded,
                                                               ElementsPerThread<2 * optimal_block_config_t::elements_per_thread>,
                                                               ElementsPerThread<optimal_block_config_t::elements_per_thread>>;
                ...
                static constexpr auto this_fft_effective_ept_v = this_fft_is_folded
                                                                     ? this_fft_ept_v / 2
                                                                     : this_fft_ept_v;
                ...
                using this_fft_block_fft_implementation =
                    typename database::detail::search_by_ept<this_fft_effective_ept_v,
                                                             this_fft_precision_t,
                ...
                static constexpr unsigned int default_block_dim_v =
                    this_fft_size_v / this_fft_effective_ept_v;
            ...
            public:
                ...
                using elements_per_thread = ElementsPerThread<this_fft_ept_v>;
                ...
                // Compute value of EPT
                using effective_elements_per_thread = ElementsPerThread<this_fft_effective_ept_v>;

                // If user set EPT, suggested FPB is for that EPT; otherwise it's for default EPT
                using suggested_ffts_per_block = CUFFTDX_STD::conditional_t<has_ept,
            ...
            private:
                ...
                // SIZE % EPT == 0
                static constexpr bool ept_is_factor_of_size = (this_fft_size_v % effective_elements_per_thread::value) == 0;
            ...
            public:
                ...
                // Effective EPT
                using effective_elements_per_thread = elements_per_thread;
            ...
            class helper
            {
...
#else
                static constexpr bool is_folded = real_fft_mode_of<Description>::value == real_mode::folded;
                static constexpr auto effective_size    = is_folded ? size_of<Description>::value / 2 : size_of<Description>::value;

                static constexpr bool is_bluestein_required_v  = is_bluestein_required<effective_size,
                ...
                static_assert(!is_bluestein_required_v || (is_bluestein_required_v && is_bluestein_supported_v),
                              "This FFT configuration is not supported");
#endif
                using selected_block_helper_t = CUFFTDX_STD::conditional_t<is_bluestein_required_v,
                                                                           helper_block_bluestein<Description>,
                                                                           helper_block_ct<Description>>;
            ...
            public:
                ...
                using effective_elements_per_thread = typename selected_block_helper_t::effective_elements_per_thread;
            ...
            class helper<Description, true>
            {
                ...
                static constexpr unsigned int dummy_thread_fft_sm_v = 900;

                static constexpr bool this_fft_is_folded = real_fft_mode_of<Description>::value == real_mode::folded;
                static constexpr auto effective_size             = this_fft_is_folded ? size_of<Description>::value / 2 : size_of<Description>::value;
                static constexpr auto this_fft_type_v            = this_fft_is_folded ? fft_type::c2c : type_of<Description>::value;

                using block_fft_record_t = database::detail::block_fft_record<effective_size,
                                                                              precision_of_t<Description>,
                                                                              this_fft_type_v,
                                                                              direction_of<Description>::value,
                ...
                using thread_fft_implementation =
                    typename database::detail::search_by_ept<effective_size,
                                                             precision_of_t<Description>,
                                                             0 /* trp_option */,
                                                             typename block_fft_record_t::blobs>::type;
            ...
            public:
                ...
                using suggested_elements_per_thread = elements_per_thread;
                using effective_elements_per_thread = ElementsPerThread<effective_size>;
        ...
        public:
            ...
            using effective_ept_type                = typename get_block_config_impl::helper<Description, is_thread_execution>::effective_elements_per_thread;
            static constexpr unsigned int effective = effective_ept_type::value;

            using suggested_ept_type                = typename get_block_config_impl::helper<Description, is_thread_execution>::suggested_elements_per_thread;
